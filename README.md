**AI PriceOptima — Dynamic Pricing Analysis**

This project analyzes ride pricing data to optimize dynamic pricing strategies.  
The tasks completed till now are data cleaning, visualization, feature engineering, and KPI tracking.

---
## 📂 Files

 Date: 15th Oct  
 Uploads:
- Milestone 1: **Cleaning_and_Visualization.ipynb** → Data cleaning, EDA, outlier & correlation analysis  
- Milestone 2: **data_ingestion_pipeline_and_KPI.ipynb** → Automated data loading + KPI analysis
- **run_pipeline.py** -> complete data ingestion pipeline in a single file
- Milestone 3: **EDA_and_feature_engineering.ipynb** → Encoding, visualization, and grouping analysis

Date: 24th Oct  
 Uploads:
- Milestone 4: **baseline_pricing_engine.ipynb** → Calculates baseline revenue from rule based on time and inventory, defines KPI

Date: 5th Nov  
 Uploads:
- Milestone 5: **model_training_and_backtesting.ipynb** → XGBoost & LightGBM model training, backtesting, and simulated revenue lift evaluation 
---

## 📊 Key KPIs
Revenue Lift (%) → ((New - Old) / Old) × 100  
Profit Margin (%) → ((Revenue - Cost) / Revenue) × 100  
Conversion Rate (%) → (Completed / Total) × 100  
 


## ⚙️ Tools Used
Python, Pandas, NumPy, Matplotlib, Seaborn, Scikit-learn


## 👩‍💻 About me
T Poojitha  
 Branch: `AI_PriceOptima_Poojitha`  

